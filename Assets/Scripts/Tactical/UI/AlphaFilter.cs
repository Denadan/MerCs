...
{
    public class AlphaFilter : MonoBehaviour, ICanvasRaycastFilter
    {
        ...
        public bool IsRaycastLocationValid(Vector2 sp, Camera eventCamera)
        {
            ...
            try
            {
                var a = _sprite.texture.GetPixel(x, y).a;
                return a > 0;
            }
            catch (UnityException e)
            {
                UnityEngine.Debug.LogError("Mask texture not readable, set your sprite to Texture Type 'Advanced' and check 'Read/Write Enabled'");
                Destroy(this);
                return false;
            }
        }
